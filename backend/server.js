...
function ensureTablesExist() {
    ...
    db.query(createInventoryTable, (err) => {
        ...
        db.query('SELECT COUNT(*) as count FROM inventory', (err, results) => {
            ...
            if (results.rows[0].count < 8) {
                // Insert standard blood types if not present
                const bloodTypes = ['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-'];
                
                bloodTypes.forEach(type => {
                    db.query('INSERT INTO inventory (Blood_Type, Quantity) VALUES ($1, $2) ON CONFLICT (Blood_Type) DO NOTHING', 
...
app.post('/api/donors', (req, res) => {
    ...
    // Insert donor
    const query = 'INSERT INTO donor (Donor_Name, DOB, Contact_Number, Blood_Type) VALUES ($1, $2, $3, $4) RETURNING Donor_ID';
    db.query(query, [name, dob, contact, bloodType], (err, result) => {
        ...
        console.log('Donor inserted successfully with ID:', result.rows[0].Donor_ID);
        const donorId = result.rows[0].Donor_ID;
        
        // Link donor to user account if user is logged in
        if (userId) {
            db.query('UPDATE users SET donor_id = $1 WHERE user_id = $2', [donorId, userId], (err) => {
                if (err) {
        ...
        const bloodQuery = 'INSERT INTO blood (Blood_Type, Donor_ID, Donation_Date) VALUES ($1, $2, $3) RETURNING Blood_ID';
        db.query(bloodQuery, [bloodType, donorId, new Date()], (err, result) => {
            if (err) {
            ...
            // Update inventory
            const updateQuery = 'UPDATE inventory SET Quantity = Quantity + 1 WHERE Blood_Type = $1';
...
app.get('/api/donors', (req, res) => {
    ...
    db.query(query, (err, results) => {
        ...
        console.log(`Fetched ${results.rows.length} donors`);
        res.json(results.rows);
    });
});
...
app.post('/api/requests', (req, res) => {
    ...
    const patientQuery = 'INSERT INTO patient (Patient_Name, Contact_Number) VALUES ($1, $2) RETURNING Patient_ID';
    db.query(patientQuery, [patientName, ''], (err, result) => {
        ...
        if (userId) {
            db.query('UPDATE users SET patient_id = $1 WHERE user_id = $2', [patientId, userId], (err) => {
                if (err) {
        ...
        const requestQuery = 'INSERT INTO bloodrequest (Blood_Type, Patient_ID, Hospital_Name, Units, status) VALUES ($1, $2, $3, $4, $5) RETURNING Request_ID';
        db.query(requestQuery, [bloodType, patientId, hospitalName, units, 'Pending'], (err, requestResult) => {
            ...
            console.log('Blood request created successfully with ID:', requestResult.rows[0].Request_ID);
            res.json({ 
...
app.get('/api/requests', (req, res) => {
    ...
    db.query(query, (err, results) => {
        ...
        const formattedResults = results.rows.map(req => ({
            id: req.id,
        ...
        console.log(`Fetched ${results.rows.length} blood requests:`, formattedResults);
...
app.patch('/api/requests/:requestId', (req, res) => {
    ...
    const getRequestQuery = 'SELECT Blood_Type, Units FROM bloodrequest WHERE Request_ID = $1';
    db.query(getRequestQuery, [requestId], (err, results) => {
        ...
        const { Blood_Type, Units } = results.rows[0];
        
        if (action === 'approve') {
            // Check if we have enough blood in inventory
            const checkQuery = 'SELECT Quantity FROM inventory WHERE Blood_Type = $1';
            db.query(checkQuery, [Blood_Type], (err, results) => {
                ...
                if (results.rows.length === 0 || results.rows[0].Quantity < Units) {
                    return res.status(400).json({ error: 'Not enough blood available in inventory' });
                }
                
                // Update request status
                const updateRequestQuery = 'UPDATE bloodrequest SET status = $1 WHERE Request_ID = $2';
                db.query(updateRequestQuery, ['Approved', requestId], (err) => {
                    ...
                    // Update inventory
                    const updateInventoryQuery = 'UPDATE inventory SET Quantity = Quantity - $1 WHERE Blood_Type = $2';
        ...
        Promise.all([
            new Promise((resolve, reject) => {
                db.query(donorQuery, (err, results) => {
                    if (err) reject(err);
                    else resolve(results.rows[0].total_donors);
                });
            }),
            new Promise((resolve, reject) => {
                db.query(inventoryQuery, (err, results) => {
                    if (err) reject(err);
                    else resolve(results.rows[0].available_units);
                });
            }),
            new Promise((resolve, reject) => {
                db.query(requestsQuery, (err, results) => {
                    if (err) reject(err);
                    else resolve(results.rows[0].pending_requests);
                });
            }),
            new Promise((resolve, reject) => {
                // Check if the donationcamp table exists first
                db.query("SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'donationcamp')", (err, results) => {
                    if (err) reject(err);
                    else if (results.rows[0].exists) {
                        db.query(campsQuery, (err, results) => {
                            if (err) reject(err);
                            else resolve(results.rows[0].upcoming_camps);
                        });
                    } else {
                        resolve(0);
                    }
...
app.get('/api/camps', (req, res) => {
    ...
    db.query("SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'donationcamp')", (err, results) => {
        ...
        if (results.rows[0].exists) {
            ...
            db.query("SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'donationcamp' AND column_name = 'Camp_Time')", (err, columns) => {
                if (err) {
                ...
                if (columns.rows[0].exists) {
                    ...
                    db.query("ALTER TABLE donationcamp ADD COLUMN Camp_Time TIME NOT NULL DEFAULT '00:00:00'", (err) => {
                        if (err) {
        ...
        function fetchCamps() {
            ...
            db.query("SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'donationcamp' AND column_name = 'Status')", (err, statusColumns) => {
                if (err) {
                ...
                if (statusColumns.rows[0].exists) {
                    console.log('Status column found, executing query');
                    executeQuery();
                } else {
                    console.log('Status column not found, adding it to table');
                    db.query("ALTER TABLE donationcamp ADD COLUMN Status VARCHAR(20) DEFAULT 'Upcoming'", (alterErr) => {
                        if (alterErr) {
                            console.error('Error adding Status column:', alterErr);
                            return res.status(500).json({ error: 'Error updating schema' });
                        }
                        
                        console.log('Added Status column to donationcamp table');
                        executeQuery();
                    });
                }
            });
            ...
            function executeQuery() {
                // Query all donation camps with all the necessary fields
                const query = `
                    SELECT 
                        Camp_ID as id,
                ...
                db.query(query, (err, results) => {
                    if (err) {
                        ...
                        // If there's still an error, use a minimal query with only guaranteed columns
                        const safeQuery = `
                            SELECT 
                                Camp_ID as id,
                                Location as location,
                        ...
                        db.query(safeQuery, (safeErr, safeResults) => {
                            if (safeErr) {
                                console.error('Error with safe query:', safeErr);
                                return res.status(500).json({ error: 'Database error with safe query' });
                            }
                            
                            // Add default values for missing fields
                            const camps = safeResults.rows.map(camp => ({
                                ...camp,
                                time: camp.time || '00:00:00',
...
app.post('/api/camps', (req, res) => {
    ...
    db.query("SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'donationcamp')", (err, results) => {
        ...
        if (results.rows[0].exists) {
            ...
            db.query("SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'donationcamp' AND column_name = 'Capacity')", (err, columns) => {
                if (err) {
                ...
                if (columns.rows[0].exists) {
                    ...
                    db.query("ALTER TABLE donationcamp ADD COLUMN Capacity INT DEFAULT 0", (alterErr) => {
                        if (alterErr) {
        ...
        function insertCamp() {
            ...
            db.query(query, [location, date, time, capacity || 0, description || ''], (insertErr, result) => {
                ...
                console.log(`Camp registered with ID ${result.rows[0].Camp_ID}`);
                res.status(201).json({
                    id: result.rows[0].Camp_ID,
...
app.delete('/api/camps/:id', (req, res) => {
    ...
    db.query("SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'donationcamp')", (err, results) => {
        ...
        if (results.rows[0].exists) {
            // Delete the camp
            const query = 'DELETE FROM donationcamp WHERE Camp_ID = $1';
...
function linkUserRecords(userId, userType, email, contactNumber, res = null) {
    ...
    if (userType === 'donor' || userType === 'both') {
        ...
        const findDonorQuery = 'SELECT Donor_ID FROM donor WHERE Contact_Number = $1 LIMIT 1';
        db.query(findDonorQuery, [contactNumber], (err, results) => {
            ...
            if (results.rows.length > 0) {
                const donorId = results.rows[0].Donor_ID;
    ...
    if (userType === 'patient' || userType === 'both') {
        ...
        const findPatientQuery = 'SELECT Patient_ID FROM patient WHERE Contact_Number = $1 LIMIT 1';
        db.query(findPatientQuery, [contactNumber], (err, results) => {
            ...
            if (results.rows.length > 0) {
                const patientId = results.rows[0].Patient_ID;
            ...
            db.query(
                ...
                (err, result) => {
                    ...
                    const token = jwt.sign(
                        { userId: result.rows[0].user_id, username, userType },
                        JWT_SECRET,
                        { expiresIn: '24h' }
                    );
                    
                    // Link user with any existing donor/patient records
                    linkUserRecords(result.rows[0].user_id, userType, email, contactNumber);
...
app.get('/api/users/profile', (req, res) => {
    ...
    try {
        ...
        // Get user profile
        const query = 'SELECT user_id, username, email, full_name, user_type, contact_number, blood_type, donor_id, patient_id FROM users WHERE user_id = $1';
    ...
    try {
        ...
        db.query(query, [decoded.userId], (err, results) => {
            ...
            if (!user.donor_id && user.matched_donor_id) {
                ...
                db.query('UPDATE users SET donor_id = $1 WHERE user_id = $2', [user.matched_donor_id, user.user_id]);
            }
            
            // Get donations
            const donationsQuery = `
                SELECT b.Blood_ID, b.Blood_Type, b.Donation_Date
                FROM blood b
                WHERE b.Donor_ID = $1
            ...
            db.query(donationsQuery, [donorId], (err, donations) => {
                ...
                console.log(`Found ${donations.rows.length} donations for donor ID ${donorId}`);
    ...
    try {
        ...
        db.query(query, [decoded.userId], (err, results) => {
            ...
            if (!user.patient_id && user.matched_patient_id) {
                ...
                db.query('UPDATE users SET patient_id = $1 WHERE user_id = $2', [user.matched_patient_id, user.user_id]);
            }
            ...
            db.query(requestsQuery, [patientId], (err, requests) => {
                ...
                console.log(`Found ${requests.rows.length} requests for patient ID ${patientId}`);
